...
const FAB = ({ onClick }: { onClick: () => void }) => (
  <motion.button
    onClick={onClick}
    className={`fixed bottom-8 right-8 bg-gradient-to-br from-blue-600 via-violet-600 to-fuchsia-500 shadow-xl text-white rounded-full p-5 
      transition border-none z-50 ring-2 ring-white/20 flex items-center justify-center hover:shadow-[0_0_40px_6px_rgba(88,80,180,0.4)]`}
    whileHover={{ scale: 1.12, boxShadow: "0 0 48px #b6baff55" }}
    whileTap={{ scale: 0.93 }}
    aria-label="Open AI Assistant"
    style={{
      boxShadow: "0 2px 32px 0 rgb(88 80 180 / 17%), 0 2px 8px 0 rgb(43 48 75 / 7%)"
    }}
  >
    <motion.span
      animate={{
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-2xl h-[92vh] bg-white bg-opacity-[0.13] shadow-2xl shadow-black/15 dark:bg-gradient-to-br dark:from-[#171928]/70 dark:via-[#252350]/90 dark:to-[#19162c]/85 border border-white/15 rounded-[2.5rem] flex flex-col overflow-hidden glass-blur relative"
      ...
      >
        ...
        <div ref={chatContainerRef} className="flex-1 p-6 space-y-6 overflow-y-auto custom-scrollbar">
          ...
          {messages.map(msg => (
            <ChatMessageItem key={msg.id} message={msg} onRegenerate={regenerateResponse} copiedMsgId={copiedMsgId} setCopiedMsgId={setCopiedMsgId} />
          ))}
        </div>
