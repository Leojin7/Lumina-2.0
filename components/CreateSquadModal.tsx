...
const CreateSquadModal = ({ isOpen, onClose }: CreateSquadModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newSquad = createSquad(name, topic);
    if(newSquad){
        onClose();
        navigate(`/squad/${newSquad.id}`);
    } else {
        alert("Failed to create squad. Please make sure you are logged in.");
    }
  };
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-4">
                <Button type="submit" className="w-full" disabled={!name.trim() || !topic.trim()}>
                  Create and Join Squad
                </Button>
              </div>
            </form>
          </motion.div>
