...
const CodingArena: React.FC = () => {
    ...
    const { addCodingSubmission, lastCodingDate } = useUserStore(state => ({
        addCodingSubmission: state.addCodingSubmission,
        lastCodingDate: state.lastCodingDate,
    }));
                            ...
                            <GlassCard delay={0.4} className="flex-grow flex flex-col">
                                <div className="p-6 border-b border-border">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <span className={`inline-flex items-center gap-2 px-3 py-1 text-sm font-semibold rounded-full ${problem.difficulty === 'Easy' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                                            problem.difficulty === 'Medium' ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' :
                                                'bg-red-500/10 text-red-400 border border-red-500/20'
                                            }`}>
                                            <div className={`w-2 h-2 rounded-full ${problem.difficulty === 'Easy' ? 'bg-green-400' :
                                        ...
                                        <div className="flex items-center gap-1 bg-muted/50 p-1 rounded-xl border border-border">
                                            {Object.entries(languageConfig).map(([lang, config]) => (
                                                <button
                                                    ...
                                                    className={`px-3 py-1 text-xs font-semibold rounded-lg transition-all duration-200 flex items-center gap-1.5 ${language === lang
