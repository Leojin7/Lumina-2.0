...
const GlassQuizCard = ({ 
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <div className={`h-full rounded-3xl border backdrop-blur-xl shadow-2xl overflow-hidden transition-all duration-300 ${
        isPremiumAndLocked 
...
const QuizzesList: React.FC = () => {
  ...
  const { 
      ...
      unlockedQuizIds: state.unlockedQuizIds,
      focusCoins: state.focusCoins,
      spendCoins: state.spendCoins,
      unlockQuiz: state.unlockQuiz,
  }));
